...
if (props.valueType==2 || props.valueType==3 || props.valueType==1){
  ...
  warpScript.value = "10000000 LIMIT { 'token' $readToken 'class' '" + props.className + "' 'labels' {} 'end' $end TOTIMESTAMP 'start' $start TOTIMESTAMP } FETCH 'data' STORE <% $data SIZE 0 == %> <% [ $readToken '" + props.className + "' {} ] FIND %> <% $data %> IFTE 'data' STORE <% $data 0 GET ATTRIBUTES $language GET 'description' STORE $data 0 GET ATTRIBUTES 'unit' GET 'unit' STORE $data 0 GET { NULL NULL 'i' 'msg/minute' 'u' $unit } SETATTRIBUTES { NULL NULL } RELABEL 'data' STORE %> <% $data 0 GET 'data' STORE %> <% %> TRY $data 10000 LTTB 'data' STORE $data VALUES SIZE 1 - 'lastValueIndex' STORE $data VALUES $lastValueIndex GET 'lastValue' STORE { 'data' $data [ SWAP bucketizer.sum 0 60000000 0 ] BUCKETIZE 'params' [ { 'datasetColor' '" + store.publicAppSettings.appPrimaryColor + "' 'xAxis' 0 } ] }"
} else if (props.valueType==4) {
  chartType.value='tabular'
  warpScript.value = "{ 'token' $readToken 'class' '" + props.className + "' 'labels' {} 'end' $end TOTIMESTAMP 'start' $start TOTIMESTAMP } FETCH 'gts' STORE [ $gts [] <% 'values' STORE $values 0 GET 'ts' STORE $values 7 GET 'value' STORE [ $ts $value 0 GET ] [ $ts NaN NaN NaN $value 0 GET ] %> MACROREDUCER ] REDUCE 'gtsResult' STORE"
}
