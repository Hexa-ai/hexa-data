...
async function init() {
  ...
  ws.value = "10000000 LIMIT { 'token' $readToken 'class' '" + refTag.value.device!.namespace + '.' + refTag.value.name + "' 'labels' {} 'end' $end TOTIMESTAMP 'start' $start TOTIMESTAMP } FETCH 'datas' STORE $datas 0 GET ATTRIBUTES $language GET 'description' STORE $datas 0 GET { NULL NULL 'i' $description } SETATTRIBUTES 'data' STORE $data 10000 LTTB 'data' STORE $data"
  ...
  if (refTag.value.device?.namespace) {
    breadCrumb.value.push(
      { name: refTag.value.device?.namespace, href: routePrefix + '/variables?namespace=' + refTag.value.device?.namespace },
    )
  }

  breadCrumb.value.push(
    { name: refTag.value.device?.namespace + '.' + refTag.value.name, href: routePrefix + '/variables/' + route.params.tagId }
  )
}
