  ...
  <div id="page" class="h-full">
    <BaseLayoutVue :pages-bread-crumb="breadCrumb" :show-tool-bar="true">
      <template v-slot:menuLeft>
        <InputSwitch class="ml-3" :text1="refTag.type == 3 ? $t('tags.warpScriptEditor') : $t('tags.javaScriptEditor')"
          :text2="''" v-model="refEditorSwitch"></InputSwitch>
        <Btn v-if="(edit && refTag.type == 3) || (!edit && refTag.type == 4 && refTag.macroUuid == null)"
          :text="$t('tags.exec')" :primary="false" :action="'play'" class="m-3" @click="play"></Btn>
        <Btn v-if="(!edit && refTag.type == 4 && refTag.macroUuid != null)" :text="$t('tags.stop')" :primary="false"
          :action="'stop'" class="m-3" @click="stopJs"></Btn>
        <p v-if="refTag.type==4 && refTag.macroUuid != null" class="m-5 h-5 px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-600 text-gray-100">
          {{$t('tags.running')}}
        </p>
        <p v-if="refTag.type==4 && refTag.macroUuid == null" class="m-5 h-5 px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-800 text-gray-100">
          {{$t('tags.stopped')}}
        </p>
      </template>
      <template v-slot:menuRight class="p-3">
        <Btn v-if="edit == true && !refEditorSwitch" :text="$t('remove')" :primary="true" class="m-3" @click="remove">
        </Btn>
        <Btn :text="edit == true ? $t('cancel') : $t('edit')" :primary="false"
          :action="edit == true ? 'cancel' : 'update'" class="m-3" @click="toggleEdit"></Btn>
        <Btn v-if="edit == true && refEditorSwitch" :text="'Sauvegarder'" :primary="false" class="m-3" @click="update">
        </Btn>
      </template>
      <template v-slot:default>
        <ComfirmPopup :title="$t('tags.macroDeleteConfirmTitle')" :cancel-text="$t('cancel')" :confirm-text="$t('remove')"
          :msg="$t('tags.macroDeleteConfirmMsg')" @comfirm="removeConfirmation" v-model="refComfirmOpen"></ComfirmPopup>
        <div :class="[!refEditorSwitch ? 'flex flex-col md:flex-row' : 'flex md:flex-row']"
          :style="'height: ' + refCodeHeight + 'px;'">
          <div class="flex-1">
            <form class="divide-gray-200 flex-grow h-full" action="#" method="POST" @submit.prevent="update">
              <warp-view-editor v-show="refEditorSwitch && refTag.type == 3" :url="warp10Url" id="wsEditor"
                :warpscript="refScript" :theme="'dark'" :horizontalLayout="false" :show-execute="false"
                :show-dataviz="false" :show-result="false" :display-messages="false"
                :config="refConfig"></warp-view-editor>
              <codemirror v-if="refEditorSwitch && refTag.type == 4" :style="{ height: '100%', margin: '10px' }"
                :disabled="!edit" v-model="refTag.script" placeholder="Code goes here..." :autofocus="true"
                :indent-with-tab="true" :tab-size="2" :extensions="refExtensions" />
                ...
                <div>
                  ...
                  <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    <div class="sm:col-span-2">
                      <InputField :title="$t('tags.name')" v-model="refTag!.name" :isRequired="true" :isDisabled="!edit"
                  ...
                  <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    <div class="sm:col-span-2">
                      <InputField :title="$t('tags.description') + ' (' + store.currentProject.l1 + ')'"
                        v-model="refTag!.descriptionL1" :isRequired="false" :isDisabled="!edit" :type="FieldType.TEXT">
                      </InputField>
                    </div>
                  </div>
                  <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    <div class="sm:col-span-2">
                      <InputField :title="$t('tags.description') + ' (' + store.currentProject.l2 + ')'"
                        v-model="refTag!.descriptionL2" :isRequired="false" :isDisabled="!edit" :type="FieldType.TEXT">
                      </InputField>
                    </div>
                  </div>
                  <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    <div class="sm:col-span-2">
                      <InputField :title="$t('tags.description') + ' (' + store.currentProject.l3 + ')'"
                        v-model="refTag!.descriptionL3" :isRequired="false" :isDisabled="!edit" :type="FieldType.TEXT">
                      </InputField>
                    </div>
                  </div>
                  <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    <div class="sm:col-span-2">
                      <InputField class="pb-3" :title="$t('tags.execInterval')" v-model="refTag!.scriptInterval"
                        :isRequired="false" :isDisabled="!edit"
                        ...
                        :values="[0, 30, 60, 600, 1800, 3600, 86400]" :index-is-value="false" :type="FieldType.SELECT">
                      </InputField>
                    </div>
                  </div>
                  <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    <div class="sm:col-span-2">
                      <InputField :title="$t('tags.type')" v-model="refTag!.type" :isRequired="false" :isDisabled="!edit"
                        :choices="[$t('tags.warpScript'), $t('tags.javascript')]" :values="[3, 4]" :index-is-value="false"
                        :type="FieldType.SELECT"></InputField>
                    </div>
                  </div>
                  <div class="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                    <div class="sm:col-span-2">
        ...
        <div v-show="refEditorSwitch && refTag.type == 3" id="warp10Debug" class=""
          ...
          <div id="warp10DebugLogs" class="w-full h-full logs overflow-y-scroll" style="background-color: #1E1E1E;">
            <div v-if="refWarp10Result != ''" :class="['w-full', 'text-white ml-3 mr-3 italic text-sm']"> Ops:
              {{ refWarp10Ops }}, Fetched: {{ refWarp10Fetched }}, Elapsed: {{ refWarp10Elapsed }}<br /></div>
            <div v-if="refWarp10ExecError" :class="['w-full', 'text-white ml-3 mr-3 italic text-sm']">Error line:
              {{ refWarp10ErrorLine }} <br /></div>
            <div v-if="!refWarp10ExecError" v-for="line of refWarp10Result" :class="['w-full', 'text-white ml-3 mr-3']">
              {{ line }} <br /></div>
            <div v-if="refWarp10ExecError" :class="['w-full', 'text-yellow-600 ml-3 mr-3']"> {{ refWarp10Result }} <br />
            </div>
          </div>
        </div>
        <div v-show="refEditorSwitch && refTag.type == 4" id="javascriptDebug" class=""
          :style="'padding:10px; width: 100%; height: ' + refDebugHeight + 'px;'">
          <div id="javascriptDebugLogs" class="w-full h-full logs overflow-y-scroll" style="background-color: #3A3F4A;">
            <div v-if="refJavascriptElapsed != ''" :class="['w-full', 'text-white ml-3 mr-3 italic text-sm']"> Elapsed:
              {{ refJavascriptElapsed }}<br /></div>
            <div v-if="!refJavascriptExecError" v-for="line of refJavascriptResult"
              :class="['w-full', 'text-white ml-3 mr-3']"> {{ line }} <br /></div>
            <div v-if="refJavascriptExecError" v-for="line of refJavascriptResult"
              :class="['w-full', 'text-yellow-600 ml-3 mr-3']"> {{ line }} <br /></div>
          </div>
        </div>
      </template>
    </BaseLayoutVue>
