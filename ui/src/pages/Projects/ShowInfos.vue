  ...
  <div>
    <BaseLayoutVue :pages-bread-crumb="breadCrumb" :show-tool-bar="true">
      <template v-slot:default>
        <div class="m-3 h-1/3">
          <h3 class="text-lg leading-6 font-medium text-gray-900">{{ $t('projectInfos.project') + ' ' +
              refProjectInfo.name
          }}</h3>
          <h3 class="text-sm leading-6 font-normal text-gray-500">{{ $t('projectInfos.owner') + ': ' }}
            <a :href="'mailto:' + refProjectInfo.owner.email"
              class="text-blue-600 visited:text-purple-600">{{ refProjectInfo.owner.email }}</a>
          </h3>
          <p class="mt-3">{{ refProjectInfo.description }}</p>
          <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-4">
            <div class="px-4 py-5 bg-white shadow rounded-lg overflow-hidden sm:p-6">
              <dt class="text-sm font-medium text-gray-500 truncate">{{ $t('projectInfos.devicesCount') }}</dt>
              <dd class="mt-1 text-3xl font-semibold text-gray-900">{{ refProjectInfo.devicesCount }}</dd>
            </div>
            <div class="px-4 py-5 bg-white shadow rounded-lg overflow-hidden sm:p-6">
              <dt class="text-sm font-medium text-gray-500 truncate">{{ $t('projectInfos.tagCount') }}</dt>
              <dd class="mt-1 text-3xl font-semibold text-gray-900">{{ refProjectInfo.tagCount }}</dd>
            </div>
          </dl>
          <div class="bg-white shadow overflow-hidden sm:rounded-md mt-3 h-2/3 z-0">
            <HdMap :deviceCollection="refDeviceCollection.data" :projectId="Number(route.params.id)"></HdMap>
          </div>
        </div>
...
const breadCrumb = ref([{ name: 'Projects', href: '/projects' }, { name: refProjectInfo.value.name, href: routePrefix }])
...
async function init() {
  ...
  breadCrumb.value = [{ name: 'Projects', href: '/projects' }, { name: refProjectInfo.value.name, href: '/projects/' + route.params.id }]
