...
async function init() {
  ...
  refUser.value = <UserModel>(
    refProject.value.users.find((element) => element.id == Number(route.params.userId))
  )
  ...
  breadCrumb.value = [
    ...
    { name: t('navigation.users'), href: routePrefix + '/' + route.params.id + '/users' },
    {
      name: refUser.value.name,
      href: routePrefix + '/' + route.params.id + '/users' + route.params.userId,
    },
  ]
}
