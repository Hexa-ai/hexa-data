<template>
  <dialog ref="refDialog" class="modal">
    <div class="modal-box w-11/12 max-w-7xl p-0 pb-16">
      ...
      <h3 class="font-bold text-lg p-4">Gestion du conteneur Node-RED</h3>
  ...
  <div class="mt-4 card card-bordered bg-base-100 shadow-md">
    <div class="card-body">
      <div class="flex justify-between items-center">
        <h2 class="card-title">Node-RED</h2>
        <div class="flex items-center text-sm">
          <label class="label cursor-pointer">
            <input v-if="edit" :disabled="loading.enable" v-model="projectService.nodeRedEnabled" @click="toggleEnabled"
              type="checkbox" class="checkbox checkbox-primary mr-2" />
            <span v-if="projectService.nodeRedEnabled" class="text-primary-500 cursor-pointer font-bold">Activé</span>
            <span v-else class="text-gray-500 cursor-pointer'">Désactivé</span>
          </label>
        </div>
      </div>

      <div>Outil de programmation visuelle basé sur des flux permettant d'effectuer le raccordement d'appareils
        matériels (IoT).</div>

      <div v-if="edit && projectService.nodeRedEnabled" class="mt-4">
        <div class="flex-col md:flex md:flex-row md:space-x-4">
          <div class="flex-1">
            <label class="block text-sm font-medium text-gray-700 mb-2">Type d'intégration de Node-RED</label>
            <select class="select select-bordered w-full mb-4" v-model="projectService.nodeRedMode">
              <option value="MANAGED">Conteneur géré par Hexa-Data</option>
              <option value="EXTERNAL">Instance Node-RED externe</option>
            </select>
          </div>
          <div class="flex-1" v-if="projectService.nodeRedMode === 'MANAGED'">
            <label class="block text-sm font-medium text-gray-700 mb-2">Version du conteneur</label>
            <input type="text" :placeholder="'Laisser vide pour utiliser la version ' + nodeRedDockerVersion"
              class="input input-bordered w-full mb-4" v-model="projectService.nodeRedVersion" />
          </div>
          <div class="flex-1" v-if="projectService.nodeRedMode === 'MANAGED'">
            <label class="block text-sm font-medium text-gray-700 mb-2">Hôte docker (host + port)</label>
            <div class="flex space-x-2">
              <input type="text" class="w-2/3 input input-bordered w-full mb-4"
                v-model="projectService.nodeRedDockerHost" />
              <input type="text" class="w-1/3 input input-bordered w-full mb-4"
                v-model="projectService.nodeRedDockerPort" />
            </div>
          </div>
          <div class="flex-1" v-if="projectService.nodeRedMode === 'EXTERNAL'">
            <label class="block text-sm font-medium text-gray-700 mb-2">URL de l'instance exerne</label>
            <input type="text" class="input input-bordered w-full mb-4" v-model="projectService.nodeRedUrl" />
          </div>
        </div>

        <div v-if="projectService.nodeRedMode === 'MANAGED'" class="mb-4">
          <label for="email" class="block text-sm font-medium text-gray-700 mb-2">Configuration
            settings.js, <a class="text-primary-500" :href="nodeRedDocumentationUrl" target="_blank">
              consulter la documentation</a></label>
          <codemirror class="w-full" :style="{ height: '10rem' }" v-model="projectService.nodeRedConfiguration"
            :indent-with-tab="true" :tab-size="2" />
        </div>

        <div v-if="projectService.nodeRedMode === 'MANAGED'" class="flex space-x-4">
          <div class="flex-1">
            <label class="block text-sm font-medium text-gray-700 mb-2">
              Mot de passe du compte
              <code>{{ nodeRedWriteUser }}</code>
            </label>
            <input type="text" class="input input-bordered w-full mb-4"
              placeholder="Laisser vide pour générer un mot de passe aléatoire"
              v-model="projectService.nodeRedWriterPassword" />
          </div>
          </div>
      </div>

      <div class="flex justify-end">
        <button v-if="edit && projectService.nodeRedEnabled" @click="submit" :disabled="loading.submit"
          class="btn btn-warning mr-4">
          <span v-show="loading.submit" class="loading loading-spinner"></span>
          <template v-if="projectService.nodeRedMode === 'MANAGED'">
            <span v-if="!project.nodeRedEnabled || projectService.nodeRedMode === 'EXTERNAL'">Déployer une nouvelle
              instance</span>
        ...
        <div class="ml-auto">
          <a v-if="project.nodeRedEnabled" :href="project.nodeRedUrl" target="_blank"
            class="btn btn-outline btn-primary mr-2">
            Accéder à l'interface
          </a>
          <button v-if="project.nodeRedEnabled" @click="showDialog"
            :disabled="!edit || projectService.nodeRedMode !== 'MANAGED'" class="btn btn-outline btn-primary">
            Gérer le conteneur
          </button>
        </div>
      </div>
...
const nodeRedDocumentationUrl = ref('https://nodered.org/docs/user-guide/runtime/configuration')
const nodeRedDockerVersion = ref(import.meta.env.VITE_DOCKER_NODERED_VERSION)
...
const updateProjectService = () => {
  projectService.value = {
    nodeRedEnabled: project.value.nodeRedEnabled || false,
    nodeRedMode: project.value.nodeRedMode || 'MANAGED',
    nodeRedUrl: project.value.nodeRedUrl || "",
    nodeRedVersion: project.value.nodeRedVersion || "",
    nodeRedDockerHost: project.value.nodeRedDockerHost || import.meta.env.VITE_DOCKER_HOST_IP,
    nodeRedDockerPort: project.value.nodeRedDockerPort || import.meta.env.VITE_DOCKER_HOST_PORT,
    nodeRedConfiguration: project.value.nodeRedConfiguration || "",
    nodeRedWriterPassword: project.value.nodeRedWriterPassword || ""
  }

  if (!project.value.nodeRedConfiguration) {
    axios.get(`projects/services/node-red/config`).then((response) => {
      projectService.value.nodeRedConfiguration = response.data.config
    })
  }
}
...
// Remove the service
const toggleEnabled = () => {
  if (projectService.value.nodeRedEnabled === true && project.value.nodeRedEnabled && project.value.nodeRedMode === 'MANAGED') {
    bus.emit('dialog.confirm', {
      message: "Souhaitez-vous vraiment supprimer le service Node-RED ? Si le service était géré par Hexa-Data, toutes les données seront définitivement supprimées.",
      ...
      onConfirm: async () => {
        ...
        try {
          await submit()
          bus.emit('toast', {
            message: "La suppression du service Node-RED a été effectuée avec succès !",
          })
          projectService.value.nodeRedEnabled = false
        } catch (err: any) {
          bus.emit('toast', {
            message: "Une erreur est survenue lors de la suppression du service Node-RED : " + err.message,
            severity: 'error',
          })
        } finally {
      ...
      onCancel: () => {
        projectService.value.nodeRedEnabled = true
      }
    })
  }
}
...
const submit = async () => {
  ...
  try {
    const response = await axios.post('projects/' + project.value.id + '/services/node-red', {
      ...projectService.value
    })
...
const refreshInfos = async () => {
  try {
    const response = await axios.get('projects/' + project.value.id + '/services/node-red/status')
...
const commands: any = ref({
  ...
  "default": {
    description: "",
    action: async () => {
      const response = await axios.post('projects/' + project.value.id + '/services/node-red/exec', {
  ...
  "stop": {
    ...
    action: async () => {
      await axios.post('projects/' + project.value.id + '/services/node-red/stop')
      await refreshInfos()
  ...
  "start": {
    ...
    action: async () => {
      await axios.post('projects/' + project.value.id + '/services/node-red/start')
      await refreshInfos()
  ...
  "restart": {
    ...
    action: async () => {
      await axios.post('projects/' + project.value.id + '/services/node-red/restart')
      await refreshInfos()
