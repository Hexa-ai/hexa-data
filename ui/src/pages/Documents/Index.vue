        ...
        >
          <div class="m-3">
            <InputField
              ...
              :type-and-size-text="''"
              id="imgBg"
              v-model="refDocument!.file"
              v-model:fileToUploadValue="fileToUpload"
              :isRequired="false"
              :isDisabled="
                !(
                  store.authUser.projectRole == RoleType.USER ||
                  store.authUser.projectRole == RoleType.EDITOR ||
                  store.authUser.isAdmin == 1 ||
                  store.currentProject.owner.id == store.authUser.id
                )
              "
              :type="FieldType.FILE"
              @update:file-to-upload-value="create"
            ></InputField>
          </div>
        </div>
...
async function init() {
  ...
  refDocumentCollection.value = await documentCrudController.index(page, 10, refSearch.value, {
    type: '0',
  })
}
...
function humanFileSize(size: number) {
  ...
  return (
    Number((size / Math.pow(1024, i)).toFixed(2)) * 1 + ' ' + ['Bytes', 'kB', 'MB', 'GB', 'TB'][i]
  )
}
</script>
