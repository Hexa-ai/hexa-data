<template>
    <BaseLayoutVue :pages-bread-crumb="[{ name: $t('users.users'), href: '/users' }]" :show-tool-bar="true">
        <template v-slot:menuLeft>
            <SearchNav class="h-full" v-model="refSearch"></SearchNav>
        </template>
        <template v-slot:menuRight class="p-3">
            <Btn :text="$t('users.newUser')" :primary="false" :action="'create'" class="m-3" @click="create"></Btn>
        </template>
...
import { ExclamationCircleIcon, CheckCircleIcon, ChevronRightIcon, MailIcon } from '@heroicons/vue/outline'
